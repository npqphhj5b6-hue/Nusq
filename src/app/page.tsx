            ...
            {featured && (
              <div className="md:col-span-2">
                <Link href={`/briefings/${featured.slug}`} className="group block">
                  <div className="border border-[#E8E5E0] rounded-2xl p-6 hover:border-[#1A4731]/40 hover:shadow-[0_4px_24px_rgba(26,71,49,0.06)] transition-all">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {featured.tags.slice(0, 2).map((tag) => (
                        <span
                          key={tag}
                          className="text-[10px] font-medium px-2.5 py-1 rounded-full bg-[#E8F0EC] text-[#1A4731]"
                        >
                          {tag}
                        </span>
                      ))}
                    ...
                    <h2
                      className="text-[1.15rem] leading-[1.35] text-[#111111] mb-4 group-hover:text-[#1A4731] transition-colors"
                      style={{ fontFamily: "var(--font-dm-serif)" }}
                    >
                      {featured.title}
                    </h2>
      ...
      {recentBriefings.length > 0 && (
        <section className="max-w-5xl mx-auto px-6 py-14">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link
              href="/briefings"
              className="text-xs text-[#737373] hover:text-[#1A4731] transition-colors"
            >
              View all →
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {recentBriefings.map((b, i) => (
              <Link key={b.slug} href={`/briefings/${b.slug}`} className="group block">
                <div className="border border-[#E8E5E0] rounded-xl p-5 h-full hover:border-[#1A4731]/40 transition-all flex flex-col">
      ...
      {essays.length > 0 && (
        <section className="border-t border-[#E8E5E0]">
          <div className="max-w-5xl mx-auto px-6 py-14">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-0 divide-y divide-[#E8E5E0] md:divide-y-0">
              {essays.slice(0, 4).map((e, i) => (
                <Link
                  key={e.slug}
                  href={`/essays/${e.slug}`}
                  className="group block py-5 border-b border-[#E8E5E0] last:border-b-0"
                >
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {e.tags.slice(0, 2).map((tag) => (
                          <span
                            key={tag}
                            className="text-[10px] px-2 py-0.5 rounded-full bg-[#E8F0EC] text-[#1A4731]"
                          >
                            {tag}
                          </span>
                        ))}
